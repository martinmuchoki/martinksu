...
STAMP="$(date +%F_%H-%M-%S)"
BACKUP_DIR="backups/intelligence_repository/$STAMP"

mkdir -p "$BACKUP_DIR" scripts services reports

echo "======================================================"
echo "MIP PRO Intelligence Repository"
echo "======================================================"

if [[ -f services/intelligence_repository.py ]]; then
    cp services/intelligence_repository.py \
       "$BACKUP_DIR/intelligence_repository.py"
fi

cat > services/intelligence_repository.py <<'PY'
...
echo
echo "==> Running repository transaction test"

venv/bin/python - <<'PY'
from services.intelligence_repository import (
    delete_intelligence_run,
...
test_context = {
    ...
    "engine_timing": {
        "prediction_center_seconds": 1.2,
        "committee_seconds": 0.01,
        "total_seconds": 1.21,
    },
}

run_id = "repository-install-test"

delete_intelligence_run(run_id)

result = save_intelligence_context(
    test_context,
    run_id=run_id,
)
...
assert result["committee_decision_count"] == 1
assert result["regime_saved"] is True

duplicate = save_intelligence_context(
    test_context,
    run_id=run_id,
)
...
deleted = delete_intelligence_run(run_id)

assert deleted is True

print("COUNTS:", get_repository_counts())
print("REPOSITORY TEST: PASS")
PY
...
git add \
    services/intelligence_repository.py \
    scripts/install_intelligence_repository.sh

if git diff --cached --quiet; then
    echo "No Git changes to commit."
else
    git commit -m \
        "add intelligence history repository"
fi

echo
echo "======================================================"
echo "MIP PRO Intelligence Repository: COMPLETE"
echo "======================================================"
echo
echo "Next: connect repository to fresh orchestrator runs."
